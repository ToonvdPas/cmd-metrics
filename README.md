...
Utility for listing the collective resource usage of all processes running with a specific executable (cmd) file.
...
```
# Install
The program `cmd-metrics` consists of two parts:

* one part which gathers process-statistics, and
* one part which gathers socket-information

The socket-part requires read-access to files in /proc/PID/fd<br>
However, non-root users do not have permission to read the files in that directory.<br>
In order to run `cmd-metrics` as a normal user with full functionality,<br>
you need to assign special capabilities to the executable.<br>
You can do this as follows:
```
# setcap CAP_DAC_READ_SEARCH,CAP_SYS_PTRACE=ep cmd-metrics
```
This is secure, al long as you make sure the program is owned by root and only writable by root, like this:
```
...
The OS-kernel on this system reports the following parameters:
        - Number of active CPU's:   4
        - Memory phys pages:        1006539
        - Memory phys pages avail:  103089
        - Memory page size (bytes): 4096
        - Memory capacity (MB):     3931
